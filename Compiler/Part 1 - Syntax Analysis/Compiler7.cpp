...
/*
  version 0:  test for reading a file/directory and eliminate comments
  version 1:  added splitting token functionality
  version 2:  added token matching and detection
  version 3:  fixed token vector functionality
  version 4:  compiler added
  version 5:  fixed string literals, properly implemented compiler
  version 6:  change < and > to their correct XML notation, &lt and &gt
  version 7:  fixed file outputs so each .jack creates its own .xml, also fixed bug with parameter
*/

const string compilerVersion = "7";
